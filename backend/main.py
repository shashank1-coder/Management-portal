...
async def create_user(user: CreateUserRequest, db: AsyncSession = Depends(get_db)):
    ...
    db_user = Users(name=user.name, username=user.username, password=hashed_password, role=user.role, email=user.email)
    db.add(db_user)
    await db.commit()
...
async def create_vendor(vendor: VendorCreate, db: AsyncSession = Depends(get_db)):
    try:
        # logger.info(f"Creating vendor: {vendor}")
        ...
        async with db.begin():
            ...
            if not organization:
                ...
                db.add(organization)
                await db.flush()  # Use flush instead of commit to keep the transaction open
                await db.refresh(organization)
            
            # logger.info(f"Organization: {organization.organization_name}")
            ...
            # logger.info(f"Role: {role.role_name}")
